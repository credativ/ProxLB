...
class Groups:
    ...
    @staticmethod
    def get_groups(guests: Dict[str, Any], nodes: Dict[str, Any]) -> ProxLbData.Groups:
        """
        ...
        for guest_name, guest_meta in guests.items():
            ...
            if not groups.affinity.get(group_name, False):
                # Create group template with initial guest meta information
                groups.affinity[group_name] = {}
                groups.affinity[group_name]["guests"] = []
                ...
                groups.affinity[group_name]["cpu_total"] = guest_meta["cpu_total"]
                groups.affinity[group_name]["cpu_used"] = guest_meta["cpu_used"]
                groups.affinity[group_name]["memory_total"] = guest_meta["memory_total"]
                groups.affinity[group_name]["memory_used"] = guest_meta["cpu_used"]
                groups.affinity[group_name]["disk_total"] = guest_meta["disk_total"]
                groups.affinity[group_name]["disk_used"] = guest_meta["cpu_used"]
            else:
            ...
            if len(guest_meta["anti_affinity_groups"]) > 0:
                for anti_affinity_group in guest_meta["anti_affinity_groups"]:
                    ...
                    if not groups.anti_affinity.get(anti_affinity_group_name, False):
                        groups.anti_affinity[anti_affinity_group_name] = {}
                        groups.anti_affinity[anti_affinity_group_name]["guests"] = []
                        ...
                        groups.anti_affinity[anti_affinity_group_name]["used_nodes"] = []
                    else:
                        groups.anti_affinity[anti_affinity_group_name]["guests"].append(guest_name)
                        groups.anti_affinity[anti_affinity_group_name]["counter"] += 1
