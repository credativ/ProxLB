...
class Nodes:
    ...
    def get_nodes(proxmox_api: ProxmoxApi, proxlb_config: Config) -> Dict[str, ProxLbData.Node]:
        ...
        for node in proxmox_api.nodes.get.model():
    ...
    def set_node_maintenance(proxmox_api: ProxmoxApi, proxlb_config: Config, node_name: str) -> bool:
        ...
        for ha_element in proxmox_api.cluster.ha.status.current.get.model():
            if ha_element.status:
                if "maintenance mode" in ha_element.status:
                    if ha_element.node == node_name:
    ...
    def get_node_rrd_data(proxmox_api: ProxmoxApi, node_name: str, object_name: str, object_type: str, spikes: bool = False) -> float:
        ...
        if spikes:
            ...
            _rrd_data_value = [row[lookup_key] for row in node_data_rrd if lookup_key in row and row[lookup_key] is not None]
